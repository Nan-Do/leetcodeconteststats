...
- **Wrong submissions** — LeetCode never publishes this number, but it charges five minutes of penalty for each one, so the gap between a user's last accepted solve and the finish time it recorded gives it back. Shown per contest in the tooltip, and a dash where the contest's own timings don't add up
